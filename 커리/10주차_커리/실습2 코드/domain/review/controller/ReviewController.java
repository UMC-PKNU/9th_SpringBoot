...
public class ReviewController implements ReviewControllerDocs {
    ...
    @Override
    public ApiResponse<List<ReviewResDTO.ReviewInfo>> getUserReviews(
            @PathVariable Long userId
    ){
        List<ReviewResDTO.ReviewInfo> reviews = reviewQueryService.getUserReviews(userId);
        return ApiResponse.onSuccess(ReviewSuccessCode.REVIEW_LIST_ME_FETCHED, reviews);
    }

    /* 02-02 특정 가게 리뷰 조회 API*/
    @GetMapping("/store/{storeId}")
    @Override
    public ApiResponse<List<ReviewResDTO.ReviewInfo>> getStoreReviews(
            @PathVariable Long storeId
    ){
        List<ReviewResDTO.ReviewInfo> reviews = reviewQueryService.getStoreReviews(storeId);
        return ApiResponse.onSuccess(ReviewSuccessCode.REVIEW_LIST_STORE_FETCHED, reviews);
    }
    ...
    @Override
    public ApiResponse<List<ReviewResDTO.ReviewInfo>> getReviewsFilter(
            @RequestParam Long userId,
            @PathVariable Long storeId,
            @RequestParam(required = false) String rating
    ){
        List<ReviewResDTO.ReviewInfo> reviews = reviewQueryService.getReviewsFilter(userId, storeId, rating);
        return ApiResponse.onSuccess(ReviewSuccessCode.REVIEW_FILTERED_LIST_FETCHED, reviews);
