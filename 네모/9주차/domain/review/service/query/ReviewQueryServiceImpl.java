...
import com.example.umc9th_week5.domain.review.code.ReviewErrorCode;
...
public class ReviewQueryServiceImpl implements ReviewQueryService {
    ...
    public ApiResponse<List<ReviewResDTO.ReviewInfo>> searchReview(Long memberId, String storeName, Float rating) {
        ...
        List<ReviewResDTO.ReviewInfo> collect = reviews.stream()
                .map(r -> ReviewConverter.toReviewInfoDTO(r))
    ...
    @Override
    public ReviewResDTO.ReviewInfoList getReview(Long memberId, int page) {
        Member member = globalValidatorService.validateExistMemberById(memberId);

        PageRequest pageRequest = PageRequest.of(page, 5);
        Page<Review> allByMember = reviewRepository.findAllByMember(member, pageRequest);

        if (allByMember.getTotalPages() < page) {
            throw new GeneralException(ReviewErrorCode.INVALID_PAGE_SIZE);
        }

        return ReviewConverter.toReviewInfoListDTO(allByMember);
    }
}
