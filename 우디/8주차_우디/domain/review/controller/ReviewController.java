...
public class ReviewController {
    ...
    @GetMapping("/{memberId}")
    public List<ReviewResDto.ReviewInfo> getMyReviews(@PathVariable Long memberId,
                                           @RequestParam(required = false) String storeName,
                                           @RequestParam(required = false) Double rating) {

        List<ReviewResDto.ReviewInfo> reviews = reviewQueryService.getReviews(memberId, storeName, rating);
