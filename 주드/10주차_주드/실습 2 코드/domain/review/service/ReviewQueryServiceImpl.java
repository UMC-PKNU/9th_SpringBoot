...
public class ReviewQueryServiceImpl implements ReviewQueryService {
    ...
    public Page<Review> getMyReviewsByStore(Long memberId, Long storeId, Integer page) {

        Member member = memberRepository.findById(memberId)
                .orElseThrow(() -> new MemberException(MemberErrorCode.MEMBER_NOT_FOUND));

        Store store = storeRepository.findById(storeId)
                .orElseThrow(() -> new ReviewException(ReviewErrorCode.STORE_ID_NOT_FOUND));

        // 3. 페이징 생성 (프론트에서 오는 page는 1부터 시작하므로 -1 처리)
        PageRequest pageRequest = PageRequest.of(page - 1, 10);

        // 4. 조회
        return reviewRepository.findAllByMemberAndStore(member, store, pageRequest);
    }
